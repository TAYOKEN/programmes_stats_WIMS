    ...
    try:
        ...
        for row in table_rows:
            ...
            if classe_val:  # Si la cellule "Classe" n'est pas vide, on considère la ligne
                ...
                if not effectif_val:
                    messagebox.showerror("Erreur", "Veuillez entrer le pourcentage correspondant à la classe.")
                    return
                try:
                    effectifs.append(float(effectif_val))
                except ValueError:
                    messagebox.showerror("Erreur", "Pourcentage invalide dans une ligne de données groupées.")
                    return
        ...
        if classes:
            ...
            if abs(total_freq - 100) > 1e-2:  # Vérifier que la somme est 100%
                messagebox.showerror("Erreur", f"La somme des pourcentages doit être égale à 100% (actuelle : {total_freq:.2f}%).")
                return

            # Moyenne groupée (calculée à partir des points milieux)
            mean = sum(m * f for m, f in zip(midpoints, effectifs)) / 100
            # Variance groupée
            variance = sum(f * ((m - mean) ** 2) for m, f in zip(midpoints, effectifs)) / 100
...
tk.Label(frame_grouped, text="Pourcentage (%)", bg="#f3f4f6", font=champ_font).grid(row=0, column=1, padx=5, pady=2)
